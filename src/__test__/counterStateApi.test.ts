...
const testUseCounterStateApi = <T>(initialState: CounterState) =>
...
describe('counterStateApi', () => {
  describe('increment', () => {
    test('adds 1 to the count', () => {
      ...
      expect(arrayState.api.value.count).toEqual(1);
      arrayState.api.increment();
      expect(arrayState.api.value.count).toEqual(2);
    });
  ...
  describe('incrementBy', () => {
    test('adds X to the count', () => {
      ...
      expect(arrayState.api.value.count).toEqual(5);
      arrayState.api.incrementBy(10);
      expect(arrayState.api.value.count).toEqual(15);
    });
    ...
    test('wont increment past max', () => {
      ...
      expect(arrayState.api.value.count).toEqual(5);
      arrayState.api.incrementBy(10);
      expect(arrayState.api.value.count).toEqual(10);
    });
  });
  ...
  describe('decrement', () => {
    test('removes 1 from the count', () => {
      ...
      expect(arrayState.api.value.count).toEqual(4);
      arrayState.api.decrement();
      expect(arrayState.api.value.count).toEqual(3);
    });

    test('wont decrement below min', () => {
      const arrayState = testUseCounterStateApi({ count: 1, min: 0 });
      arrayState.api.decrement();
      expect(arrayState.api.value.count).toEqual(0);
      arrayState.api.decrement();
      expect(arrayState.api.value.count).toEqual(0);
    });
  });
