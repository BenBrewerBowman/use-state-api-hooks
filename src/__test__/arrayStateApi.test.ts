...
describe('arrayStateApi', () => {
  describe('clear', () => {
    test('clears the array state', () => {
      ...
      expect(arrayState.api.value).toEqual([]);
    });
  });
  ...
  describe('push', () => {
    test('pushes to the end of the array', () => {
      ...
      expect(arrayState.api.value).toEqual([1]);
      arrayState.api.push(2);
      expect(arrayState.api.value).toEqual([1, 2]);
    });
  });

  describe('pop', () => {
    test('pops values from the end of the array', () => {
  ...
  describe('shift', () => {
    test('pops off one element from the start of the array', () => {
      ...
      expect(arrayState.api.value).toEqual([2, 3]);
      arrayState.api.shift();
      expect(arrayState.api.value).toEqual([3]);
      arrayState.api.shift();
      expect(arrayState.api.value).toEqual([]);
    });
  ...
  describe('unshift', () => {
    test('pushes elements onto the start of the array', () => {
      ...
      expect(arrayState.api.value).toEqual([3, 4]);
      arrayState.api.unshift(1, 2);
      expect(arrayState.api.value).toEqual([1, 2, 3, 4]);
    });
  });
  describe('reverse', () => {
    test('reverses an array', () => {
      ...
      expect(arrayState.api.value).toEqual([5, 4, 3, 2, 1]);
      arrayState.api.reverse();
      expect(arrayState.api.value).toEqual([1, 2, 3, 4, 5]);
    });
  });

  describe('insertAt', () => {
    test('inserts values into an array at index', () => {
      const arrayState = testUseArrayStateApi(['Banana', 'Pineapple']);
      arrayState.api.insertAt('Mango', 1);
      expect(arrayState.api.value).toEqual(['Banana', 'Mango', 'Pineapple']);
    ...
    test('inserting out of bounds does nothing', () => {
      ...
      expect(arrayState.api.value).toEqual([1, 2, 3]);
  ...
  describe('upsertAt', () => {
    test('replaces value of array at index', () => {
      const arrayState = testUseArrayStateApi(['Apple', 'Peach', 'Pear']);
      arrayState.api.upsertAt('Grapefruit', 1);
      expect(arrayState.api.value).toEqual(['Apple', 'Grapefruit', 'Pear']);
      arrayState.api.upsertAt('Banana', 0);
      expect(arrayState.api.value).toEqual(['Banana', 'Grapefruit', 'Pear']);
    ...
    test('upserting out of bounds does nothing', () => {
      const arrayState = testUseArrayStateApi(['Apple', 'Peach', 'Pear']);
      arrayState.api.upsertAt('Grapefruit', -1);
      expect(arrayState.api.value).toEqual(['Apple', 'Peach', 'Pear']);
  ...
  describe('deleteAt', () => {
    test('deletes the element at index', () => {
      ...
      expect(arrayState.api.value).toEqual(['Banana', 'Peach', 'Pear']);
      arrayState.api.deleteAt(2);
      expect(arrayState.api.value).toEqual(['Banana', 'Peach']);
    });

    test('does nothing if delete index is out of bounds', () => {
      const arrayState = testUseArrayStateApi(['Apple', 'Banana']);
      arrayState.api.deleteAt(2);
      expect(arrayState.api.value).toEqual(['Apple', 'Banana']);
